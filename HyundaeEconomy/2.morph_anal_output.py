...
import ujson
import os
import winsound # 다 끝나면 소리가 나게 만들어봤다. 근데 이건 양이 적어 그다지 필요없다.

inputfileList = []
outputFileList = []
inputFolder = 'HyundaeEconomy/ready/'
outputFolder = 'HyundaeEconomy/morphed/'
...
def print_morph_anal(text, morph_anal, output):
    """
    형태소 분석 결과를 출력한다.
    ...
    for i in range(len(inputfileList)):
        ...
        for line in f:
            ...
            for sentence in sentences:
                ...
                output = print_morph_anal(sentence, morph_anal,output)

            result = ujson.dumps(output, ensure_ascii=False)
        w.write(result)   
    f.close()
    w.close()   
