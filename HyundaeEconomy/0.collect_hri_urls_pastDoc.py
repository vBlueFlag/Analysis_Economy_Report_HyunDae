...
1. 현대 경제 연구원의 보고서들을 모두 크롤링한다.
2. 0.collect_hri_urls 파일은 현대경제연구원의 최신보고서 주소 형식에 맞춰 만든 것이다.
3. 0.collect_hri_urls_pastDoc.py 파일은 과거 보고서 다운 주소를 가지고 있다. 
   해당 사이트의 보고서 주소 방식이 바뀌었기 때문에 어쩔 수 없이 이것도 만들었다.
   과거 보고서를 다운 받을 때 쓴다.
...
def ext_informPDF(html,i):
    ...
    try:
        ...
        reportURL = "https://hri.co.kr:442/download.asp?file_path=%2Fpublication%2F&file_name="+ urllib.parse.quote_plus(reportID)
...
def main():
    ...
    try:
        ...
        for i in range(797,len(information)):  #특정 번호의 리포트부터 가져오고 싶을 때는 range(번호,len(information)) 로 바꾼다.
