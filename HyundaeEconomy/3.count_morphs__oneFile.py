...
import os
inputfileList = []
inputFolder = 'HyundaeEconomy/morphed/'
outputFolder = 'HyundaeEconomy/ranked/'
...
for root, dirs, files in os.walk(inputFolder): # 입력 폴더에서 파일리스트를 받아
    for file in files:
        ...
        outputFileList = outputFolder + 'hri.txt'
MORPH = "morphAnal"
...
def main():
    ...
    w = open(outputFileList,'a', encoding="utf-8")
    
    for i in range(len(inputfileList)):
        f = open(inputfileList[i],'r', encoding="utf-8") 
        morphed_text = ujson.load(f)
        for j in range(len(morphed_text)):
            title_morphs = ext_morphs(morphed_text[j][MORPH])
