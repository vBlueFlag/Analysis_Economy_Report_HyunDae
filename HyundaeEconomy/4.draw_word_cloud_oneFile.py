...
inputFolder = 'HyundaeEconomy/ranked/'
outputFolder = 'HyundaeEconomy/result_wordcloud/'

if not os.path.isdir(outputFolder): # 결과 출력 폴더가 없다면
    os.mkdir(outputFolder)  # 폴더를 만든다

inputfileList= inputFolder + 'hri.txt' # 리스트로 만든다.
outputFileList= outputFolder + 'hri.png'
...
def read_word_counts(word_count_file_name):
    ...
    with open(word_count_file_name, "r", encoding="utf-8") as word_count_file:
        word_num = 0
